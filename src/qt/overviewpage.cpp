                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in abet.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in abet.conf");
    }
    ...
    // ABET Available
    bool showABETAvailable = settingShowAllBalances || pivAvailableBalance != nTotalBalance;
    bool showWatchOnlyABETAvailable = showABETAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ...
    ui->labelWatchAvailable->setVisible(showWatchOnlyABETAvailable && showWatchOnly);

    // ABET Pending
    bool showABETPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyABETPending = showABETPending || watchUnconfBalance != 0;
    ...
    ui->labelWatchPending->setVisible(showWatchOnlyABETPending && showWatchOnly);

    // ABET Immature
    bool showABETImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showABETImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showABETImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showABETImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // ABET Locked
    bool showABETLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyABETLocked = showABETLocked || nWatchOnlyLockedBalance != 0;
    ...
    ui->labelWatchLocked->setVisible(showWatchOnlyABETLocked && showWatchOnly);
